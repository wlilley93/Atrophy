...
The companion's memory is a three-layer architecture stored in a per-agent SQLite database at `agents/<name>/memory.db`. Each layer serves a different purpose, and they work together to give the agent continuity across sessions.
...
Each agent has its own database at `agents/<name>/memory.db`. There is no shared memory between agents.
