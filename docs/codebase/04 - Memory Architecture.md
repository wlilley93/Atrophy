...
The companion's memory is a three-layer SQLite system defined in `db/schema.sql` and operated through `core/memory.py`. Database is per-agent at `~/.atrophy/agents/<name>/data/memory.db`. WAL mode is enabled for concurrent reads.
